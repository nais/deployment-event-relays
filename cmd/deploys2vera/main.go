...
func init() {
	...
	flag.StringVar(&cfg.URL, "vera-url", cfg.URL, "Full URL to vera api")
...
// Execute incoming work (post data to Vera) and retry indefinitely.
func worker(work chan Workload) {
	...
	for workload := range work {
		...
		for {
			...
			if err == nil {
				workload.Logger.Infof("Wrote data to Vera")
				break
			}
			transientErrors.Inc()
...
func run() error {
	...
	// Create a goroutine that will do the actual posting to Vera.
	go worker(workloads)
	...
	for {
		...
		case msg, ok := <-kafka.Consume():
			...
			// Create a callback function that submits data to Vera and acknowledges
